#!/bin/zsh
# Round 10 (addendum 11) §4 I gates at executable head 31deed8 (last .rs commit; step 3 was comments/docs only).
# Raw logs live OUTSIDE the tree in /tmp/w-remit-r10/logs/ (nothing >200 lines under reports/); summaries under reports/w-remit-r10/.
# Each log: line 1 = `git rev-parse HEAD`, line 2 = exec head + .rs diff to HEAD (must be empty), line 3 = the command.
set -u
cd "$(dirname "$0")/../.." || exit 2
export PATH="$HOME/.cargo/bin:$PATH"
EXEC=31deed8
L=/tmp/w-remit-r10/logs
mkdir -p $L
...
# money-path binary x3, CI's exact command (.github/workflows/ci.yml money-path job)
for i in 1 2 3; do
