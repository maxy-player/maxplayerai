...
pub fn run(args: &[String], out: &mut dyn Write, err: &mut dyn Write) -> i32 {
    ...
    // `maxplayer seller fees` — the platform fee journal (read-only) and, under `fees remit`, the one
    // explicit command that pays the accrued fee out (dry run by default, `--confirm` to pay).
    // Dispatched before the option parser because it takes no seller options and never boots a seat.
